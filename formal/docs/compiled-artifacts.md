...
> **Status (2026-07-03).** Kept as design history; two operational details
> changed since this was written. (1) The Rust exporter now emits DATA ONLY —
> the single-module `coq-typed` format was removed and no proof text is ever
> generated; the formats are `json`, `coq` (byte data module,
> `CompiledMultisigByteData.v`), and `coq-typed-split <output_dir>` (the full
> data-module family, regenerated byte-identical to the checked-in files).
> Proof modules are hand-maintained in `formal/`. (2) The artifact chain has
> since been completed well past this inventory: byte-exact real-SHA-256 CMR,
> a discharged checked run, and in-Coq execution of the deployed bytes — see
> `README.md` for the current claim set.
